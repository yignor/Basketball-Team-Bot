...
def _train_screen(period: str = "") -> Tuple[str, InlineKeyboardMarkup]:
    ...
    # Отметка стоит, а суммы нет — самый тихий способ потерять деньги: такой
    # человек выглядит занимающимся, но должником не считается вовсе, потому
    # что считать нечего. За игры бот хотя бы подставит типовую цену команды,
    # за тренировки — нет.
    import coach_payments as _cp
    mute = [p for p in _cp.players()
            if p["pays_season"] and not int(p.get("pay_season") or 0)]
    if mute:
        lines += ["", "⚠️ Отметка активности стоит, а взнос не проставлен у "
                      f"{len(mute)}: " + ", ".join(p["title"] for p in mute[:8])
                      + ("…" if len(mute) > 8 else "")
                      + ". С них взнос не ждём — проставь сумму в «👥 Игроки»."]
