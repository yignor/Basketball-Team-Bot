...
# Отказ формулируют по-разному, и список слов эту войну проигрывает:
# «Не смогу»/«Пропускаю» проходили как ЯВКА. Для опросов по дням отчёты теперь
# опираются на другой признак (назван ли день, см. attendance_summary), а тут
# держим расширенный набор — он влияет на vote_type в журнале.
VOTE_ABSENT_WORDS = {"нет", "не приду", "no", "не могу", "не смогу", "не буду",
                     "пропущу", "пропускаю", "не в этот раз", "никак", "отдыхаю"}
