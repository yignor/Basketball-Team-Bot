...
- пороги СМЕЩЕНЫ друг относительно друга (гистерезис): войти в серебро — 7
  очков за игру, а выпасть из него — только ниже 5. Иначе игрок на границе
  прыгал бы туда-обратно после каждой игры.

Пороги посчитаны из той же кривой, которой считались стартовые цены: 7 / 13 /
19 / 26 фэнтези-очков за игру — это ровно цены 15 / 30 / 50 / 70. То есть
«потолок ранга» и «нижняя цена следующего» — одно и то же число, просто в
разных единицах.

Шкала пересчитана 30.07.2026, когда в формулу добавились промахи, фолы и
дабл-даблы: на боевых играх наших команд новая оценка вышла ≈ 0.92·старой −
1.8 (штрафы за брак весят больше, чем добавляют редкие дабл-даблы). Пороги
сдвинуты ровно на столько же — иначе вся команда съехала бы на ранг вниз не
потому, что стала играть хуже, а потому что поменяли линейку.
...
# Лестница ровная: каждый следующий ранг — плюс ~6.5 очков за игру
# (7/13/19/26), а порог падения на 3 ниже своего порога входа. Этот зазор и
# есть разница между полом и потолком: на границе игрок не прыгает туда-обратно.
RANKS: Dict[str, Dict[str, Any]] = {
    BRONZE:   {"low": 5,  "high": 14,  "up": 7.0,  "down": None},
    SILVER:   {"low": 15, "high": 29,  "up": 13.0, "down": 5.0},
    GOLD:     {"low": 30, "high": 49,  "up": 19.0, "down": 10.0},
    PLATINUM: {"low": 50, "high": 69,  "up": 26.0, "down": 16.0},
    ELITE:    {"low": 70, "high": 100, "up": None, "down": 22.0},
}
...
def _price_for_fp(fp: float) -> int:
    ...
    Между узлами (7→15, 13→30, 19→50, 26→70) считаем линейно: точнее кривой
    здесь не нужно, цена всё равно двигается шагом."""
    points = [(0.0, 5), (7.0, 15), (13.0, 30), (19.0, 50), (26.0, 70), (35.0, 100)]
    if fp <= points[0][0]:
        return points[0][1]
