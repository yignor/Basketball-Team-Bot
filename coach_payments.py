...
# Взнос за турнир целиком — третий вид, живёт в season_fees. Здесь он нужен
# только чтобы сводки и экран удаления не показывали «?» вместо подписи.
KIND_TOURNEY = "tourney"
...
def own_game_price(player: Optional[Dict[str, Any]] = None) -> int:
    """Цена игры, проставленная ЭТОМУ человеку. Не проставлена — ноль.

    Отдельно от game_price намеренно. Там подстановка типовой цены уместна:
    надо классифицировать сумму из СМС, когда человек ещё не опознан. А вот
    решая, сколько человек должен, выдумывать за тренера нельзя: он работает
    через бота, и убранная цена — это его решение «с этого не берём», а не
    пропуск. За тренировки так и было всегда; за игры бот подставлял 900 ₽ и
    требовал их с того, у кого цена стёрта."""
    return int((player or {}).get("pay_game") or 0)
