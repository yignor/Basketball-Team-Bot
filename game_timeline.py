...
#
# Промахи (4/5/6) в моменты не берём: смотреть свои промахи никто не просил.
...
# день чуть позже — в моменты добавились потери и фолы, и по той же причине.
...
REDO_BEFORE = "2026-08-17T12:30:00"
...
def format_moments(source: str, game_id: Any, player_id: Any,
    ...
    Потери и фолы здесь есть — их пересматривают, чтобы понять, что пошло не
    так. А промахов нет: бросок мимо видно и по статистике, отдельная ссылка
    на каждый превратила бы список в перечень неудач."""
