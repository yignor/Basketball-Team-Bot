...
def test_required_fields_guarded() -> None:
    ...
    check("{месяц}" in why, f"сказано, каких именно не хватает: {why}")
    # Сумма в вопросе НЕ обязательна: это предварительный опрос, а не счёт.
    check("{сумма}" not in why, "сумму в вопросе не требуем")
...
def test_real_messages_use_templates() -> None:
    ...
    check("Категорически приветствую" in builtin, "с приветствия и начинается")
    check("5500" not in builtin,
          f"суммы в предварительном опросе нет:\n{builtin}")
    ...
    check("5500" in changed, "а захочет тренер вернуть сумму — подставится")
    mt.reset("ask")
