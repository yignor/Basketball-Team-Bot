...
FORBIDDEN = re.compile(
    ...
    # Уход и возврат пишут в лист; «отчества» ходят в лигу и пишут в лист.
    r"|:field:(leave2|back):|coach:patro"
    # Разбор записи: save/who пишут заметки в настоящую базу, out шлёт
    # тренеру разбор сообщениями.
    r"|coach:vn:(save|who:|out:)"
    # Закрытие лиги меняет то, что бот считает текущим сезоном: обходу такое
    # нажимать нельзя. Сам вопрос «закрыть?» — можно, он ничего не меняет.
    ...
    # Команды в лигах: «ok» включает слежение (опросы в общий чат), «rm» его
    # выключает. Обходу такое нажимать нельзя.
    r"|coach:tm:(ok|oks|rm:|rms:)"
    # Маршруты топиков: chat/off меняют, куда уходят сообщения всей команде.
    r"|coach:rt:(chat|off|close2|open):"
    # Выключатели: обход не должен молча выключить опросы живой команде.
    r"|coach:ft:"
